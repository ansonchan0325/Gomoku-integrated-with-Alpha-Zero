...
class MCTS(object):
    ...
    def _playout(self, state):
        ...
        List_ActionPlusProb, leaf_value = self._policy(state)
        ended, winner = state.game_end()
        if not ended:
            node.expand(List_ActionPlusProb)
        elif ended and winner == -1:
            leaf_value = 0.0
        elif ended and winner == state.get_current_player():
            leaf_value = 1.0
        else:
            leaf_value = -1.0
    ...
    def get_move_probs(self, state, temp=1e-3):
        ...
        """
        # TODO: program here
        # playout for n times in a loop
        for n in range(self._n_playout):
            state_copy = copy.deepcopy(state)
            self._playout(state_copy)

        #get n_visits for children nodes of root node
        act_visits = [(act, node._n_visits)
                      for act, node in self._root._children.items()]
        acts, visits = zip(*act_visits)

        #calculate action probabilities with softmax function
        #    act_probs = softmax(1.0/temp * log(n_visits + 1e-10))
    ...
    def get_action(self, board, temp=1e-3, return_prob=0):
        ...
        if len(sensible_moves) > 0:
            ...
            if self._is_selfplay:
                p1 = 0.75 * probs
                p2 = 0.25 * np.random.dirichlet(0.3 * np.ones(len(probs)))
                move = np.random.choice(acts, p = p1 + p2)
